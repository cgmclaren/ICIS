...
#define SQL_getNID "select MIN(NID) from NAMES"

static  CODBCbindedStmt &local  = _localDBC->BindedStmt(SQL_getNID); 

static LONG localNID=0,nextNID=0;
   ...
   if (fLocal )
   {
      if (!localNID)
      {
			...
			source1.Execute();
			source1.Fetch();
			localNID = source1.Field(1).AsInteger();

	        local.Bind(1,nextNID);
      }
...
//////////////////////////////////////////////////////////////////////////////
//Description: Gets the preferred ID of a germplasm where NSTAT=8

//////////////////////////////////////////////////////////////////////////////
LONG getPrefID(BOOL fLocal,GMS_Name &name, LPSTR szName, LONG nszName
               , LONG fSearchOption)
{
#define SQL_GETPREFID "\
   select GID, NTYPE, NSTAT, NUID, NVAL, NLOCN, NDATE, NREF, NID \
   from NAMES \
   where GID=? and NSTAT=8 "

   PUSH(getPrefID);

static CODBCbindedStmt &central= _centralDBC->BindedStmt(SQL_GETPREFID);
static CODBCbindedStmt &local  = _localDBC->BindedStmt(SQL_GETPREFID);
       CODBCbindedStmt *source;
...
string strName;
long lngSize, lngCurSize=GMS_NVAL_SIZE;
static long j=0;
static bool isOracle=false;
...
BOOL  fOK;
   ...
   if (fSearchOption==FIND_FIRST)
   {
	  ...
	  if (!isOracle) 
		   ...
		   {
				BINDPARAMS(1,nval1,lngSize);
				BINDPARAMS(2,nval2,lngSize);
				BINDPARAMS(3,nval3,lngSize);
				BINDPARAMS(4,nval4,lngSize);
		  }

	  //++j;
	  //if ((j%20) == 0 )
   //	     LOGF("Number of times called %d\n", j);
 	  LOG("Execute SQL \n");
      local.Execute();
      central.Execute();
   }
   ...
   while (TRUE)
   {
      ...
      if (fOK=source->Fetch())
      {
         ...
         {
            ret = getGermplasmRecord(_name.gid<0, _name.gid, _germ);
			LOG("Exit the call to getGermplasmRecord\n");
            if (ret!=GMS_SUCCESS && ret!=GMS_NO_DATA)
            {
               POP();
            }
			...
			else if (_germ.grplce!=0) {
               continue;
              name.gid=_germ.gid;
   			  LOG("In the condition _germ.grplce!=0\n");
			}
		...
		// Check if it has been replaced or deleted then ignore it.
   		LOG("Before the getReplacementGermplasmFromChanges\n");
		LONG replacement;
		if (getReplacementGermplasmFromChanges(_name.gid,replacement)!=GMS_SUCCESS)
			break;
      }
...
//////////////////////////////////////////////////////////////////////////////
//::listNames2
//Modified: Feb 19, 2008(AMP) - Preferred ID NSTAT=8 is checked if no preferred Name NSTAT=1
//////////////////////////////////////////////////////////////////////////////
LONG listNamesEx(LONG gid,CHAR* szBuffer, LONG nszBuffer,LPCSTR szDelimeter)
{
   ...
   *szBuffer='\0';
   ...
   if (ret!=GMS_NO_DATA)
      abuffer.push_back(szName);
   // Then the rest, ignoring Preferred and Alternative names and PreferredID
   n.ntype=0;
   n.nstat=0;
   ret=GMS_getName(&n,szName,MAX_STR,FIND_FIRST);
   if (ret!=GMS_SUCCESS)
...
string input=szInput;
   ...
   if (((i=input.find_first_of('-')) !=-1 ) && (i > 0 && (i < input.length()-1)))
...
//////////////////////////////////////////////////////////////////////////////
//Description: The function returns the preferred ID (NSTAT=8)  of a germplasm. 
//////////////////////////////////////////////////////////////////////////////
LONG DLL_INTERFACE GMS_getPreferredID(GMS_Name *data, LPSTR szName, LONG nszName
               , LONG fSearchOption)
{
   static BOOL fLocal=TRUE;
   LONG  nid=0,to=0, gid=0, nstat=0;
   
   PUSH(GMS_getPreferredID);
   ...
   if (fLocal){
      ret=getPrefID(LOCAL,*data,szName,nszName,fSearchOption);
      if (ret!=GMS_SUCCESS){
         fLocal = FALSE; 
         ret=getPrefID(CENTRAL,*data,szName,nszName,FIND_NEXT);
      }
   }
   else
      ret=getPrefID(CENTRAL,*data,szName,nszName,fSearchOption);
   if (ret == GMS_SUCCESS)
   {
      nid = data->nid;
	  nstat= data->nstat;
...
//////////////////////////////////////////////////////////////////////////////
//Description: The function returns the preferred name of a germplasm. If there is no 
//             preferred name, the preferred ID will be returned.
//////////////////////////////////////////////////////////////////////////////
LONG DLL_INTERFACE GMS_getPreferredName(GMS_Name *data, LPSTR szName, LONG nszName
               , LONG fSearchOption)
{
   static BOOL fLocal=TRUE;
   LONG  nid=0,to=0, gid=0, nstat=0;
   
   PUSH(GMS_getPreferredName);
   ...
   if (fLocal){
      ret=getPrefName(LOCAL,*data,szName,nszName,fSearchOption);
      if (ret!=GMS_SUCCESS){
         fLocal = FALSE; 
         ret=getPrefName(CENTRAL,*data,szName,nszName,FIND_NEXT);
      }
   }
   else
      ret=getPrefName(CENTRAL,*data,szName,nszName,fSearchOption);
   if (ret == GMS_SUCCESS)
   {
      nid = data->nid;
	  nstat= data->nstat;
	  ...
	  getNameFieldFromChanges(nid,"NSTAT",data->nstat);
	   ...
	   ret = GMS_getPreferredID(data,szName, nszName, FIND_FIRST);
   }
...
LONG getNextSequence(LONG ntype, LPSTR szPrefix, LONG *nextSequence)
{
#define SQL_GETNEXTSEQUENCE "\
    SELECT Max(CLng((Mid([NVAL],Len(?)+2)))) AS Expr1 \
	FROM [NAMES] \
	WHERE (((([NVAL])) Like ? )) \
	GROUP BY NAMES.NTYPE \
	HAVING (((NAMES.NTYPE)=?)) \
	ORDER BY Max(CLng((Mid([NVAL],Len(?)+2)))) "


#define SQL_GETNEXTSEQUENCE_MYSQL "\
	SELECT max( CONVERT(Mid(NVAL,Length(?)+2),UNSIGNED) ) AS Expr1 \
	FROM NAMES \
	WHERE NVAL Like ? \
	GROUP BY NAMES.NTYPE \
	HAVING NAMES.NTYPE=? \
    ORDER BY max( CONVERT( Mid(NVAL,Length(?)+2),UNSIGNED)) "

#define SQL_GETNEXTSEQUENCE_POSTGRES "\
	SELECT MAX(CAST(substring(NVAL from STRPOS(?,NVAL) + 6 for length(nval)) AS INTEGER)) AS Expr1 \
	FROM NAMES \
	WHERE NVAL Like ? \
	GROUP BY NAMES.NTYPE \
	HAVING NAMES.NTYPE=? \
	ORDER BY MAX(CAST(substring(NVAL from STRPOS(?,NVAL) + 6 for length(nval)) AS INTEGER))"
 


 //SELECT Max(CLng((Mid([NVAL],Len('PI')+2)))) AS Expr1  \
 //FROM [NAMES]  \
 //WHERE (((([NVAL])) Like 'PI*'))  \
 //GROUP BY NAMES.NTYPE  \
 //HAVING (((NAMES.NTYPE)=1))  \
 //ORDER BY Max(CLng((Mid([NVAL],Len('PI')+2)))) "

	PUSH(getNextSequence);

static CODBCbindedStmt &central= _centralDBC->BindedStmt(SQL_GETNEXTSEQUENCE);
static CODBCbindedStmt &local  = _localDBC->BindedStmt(SQL_GETNEXTSEQUENCE);
       CODBCbindedStmt *source;

static CHAR _nval[GMS_NVAL_SIZE+1],_prefix[GMS_NVAL_SIZE];
static LONG _nextSequence, _ntype;
static BOOL first_time=TRUE;
...
static string central_sql, local_sql;
      ...
      strcpy(_prefix,szPrefix);
	  strcpy(_nval,szPrefix);
      strcat(_nval,"%");
	  _ntype = ntype;
      if (first_time)
      {
	     szDB = _localDBC->DatabaseName();
	     if (strcmp(szDB, "PostgreSQL")==0 ) {
	        local_sql=SQL_GETNEXTSEQUENCE_POSTGRES;
		 }
		 else if  (strcmp(szDB, "MySQL")==0) 
	        local_sql=SQL_GETNEXTSEQUENCE_MYSQL;
		 else
		    local_sql=SQL_GETNEXTSEQUENCE;
		 local.SetSQLstr(local_sql.c_str());

	     szDB = _centralDBC->DatabaseName();
	     if (strcmp(szDB, "PostgreSQL")==0 ) {
	        central_sql=SQL_GETNEXTSEQUENCE_POSTGRES;
		 }
		 else if  (strcmp(szDB, "MySQL")==0) 
	        central_sql=SQL_GETNEXTSEQUENCE_MYSQL;
		 else
		    central_sql=SQL_GETNEXTSEQUENCE;
		 central.SetSQLstr(central_sql.c_str());

		 BIND(1,_nextSequence);
         BINDPARAMS(1,_prefix, GMS_NVAL_SIZE);
         BINDPARAMS(2,_nval, GMS_NVAL_SIZE);
         BINDPARAM(3,_ntype);
         BINDPARAMS(4,_prefix, GMS_NVAL_SIZE);

		 first_time = FALSE;
      }
   //MessageBox(NULL,local.SQLstr(),"ICIS V5",MB_OK);
   ...
   *nextSequence = 0;
   ...
   if (ret==GMS_SUCCESS) *nextSequence = *nextSequence + 1;

   POP();

#undef SQL_GETNEXTSEQUENCE
}

LONG DLL_INTERFACE GMS_getNextSequence(LONG ntype, LPSTR szPrefix, LONG *nextSequence)
{
	PUSH(GMS_getNextSequence);
	ret = getNextSequence(ntype, szPrefix,nextSequence);
	POP();
}
...
{
	LONG nextSequence;
	CHAR _nval[GMS_NVAL_SIZE+1];
	PUSH(GMS_getNextSequence);
	ret = getNextSequence(ntype, szPrefix,&nextSequence);
    itoa(nextSequence,_nval,10);
	strcpy(szNextName,szPrefix);
	strcat(szNextName," ");
	strcat(szNextName,_nval);
	POP();
}
