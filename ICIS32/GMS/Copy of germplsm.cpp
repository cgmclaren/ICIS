...
{
	//MessageBox(NULL,"GETNEXTGID","GERMPLASM",MB_OK);
   static LONG localGID=0,centralGID=0;
  

   if (fLocal)
   {
	  if (!localGID)  //- commented out by AMP: 2007/10/17
      {
	  ...
	  source2.Execute();
      source2.Fetch();
      localGID=min(localGID,source2.Field(1).AsInteger());
   }
   //else if (!fLocal && !centralGID) //- commented out by AMP: 2007/10/17
   //{
   //   CODBCdirectStmt source = _centralDBC->DirectStmt("select MAX(GID) from GERMPLSM") ;
   //   source.Execute();
   //   source.Fetch();
   //   centralGID=source.Field(1).AsInteger();
   //}
   //return (fLocal)? --localGID:++centralGID;
	return --localGID;
}
...
LONG addGermplasm(BOOL fLocal, GMS_Germplasm &germ,GMS_Name &name,CHAR *szName,LONG *progenitors)
{
   PUSH(addGermplasm);
   CHAR szCommit[8];
   static bool fCommit=FALSE, first_time=TRUE;
   if (first_time) { //check if AUTOCOMMIT=YES in SetGEN
	  char szIniFile[150];
...
LONG DLL_INTERFACE GMS_deleteProgenitorID(LONG gid)
{
   ...
   if (UACCESS<30) 
      ...
      ret= deleteProgenitorID(gid<0,gid);
   POP();
}
...
#define SQL_findSID  "\
	   SELECT GERMPLSM.GID  \
       FROM GERMPLSM                                   \
       WHERE (GERMPLSM.CID=?)  AND (GERMPLSM.SID=?) "
