...
LONG getNextGID(BOOL fLocal)
{
   ...
   if (fLocal && !localGID)
   {
      ...
      source2.Execute();
      source2.Fetch();
      localGID=min(source1.Field(1).AsInteger(),source2.Field(1).AsInteger());
   }
   else if (!fLocal && !centralGID)
   {
...
#define SQL_findDerivative  "\
	   SELECT GERMPLSM.GID  \
       FROM GERMPLSM                                   \
       WHERE (GERMPLSM.GPID2=?)  AND (GERMPLSM.GNPGS<0) "
...
{
   LONG ret1, ret2, gid2, _dgid, _nstep, derivativeList[100], cntList=0,j;
