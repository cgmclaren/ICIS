/* Persons.cpp : Implements the functions to access and manipulate the PERSONS table of ICIS
...
//////////////////////////////////////////////////////////////////////////////
//  Finds the minimum institutid
//////////////////////////////////////////////////////////////////////////////
LONG getNextInstitutID(void)
{
static LONG institutID=0;

   if (!institutID)
   {
      CODBCdirectStmt local = _localDMS->DirectStmt("SELECT MIN(INSTITID) FROM INSTITUT");
      local.Execute();
      local.Fetch();
      if (!local.NoData())
         institutID=local.Field(1).AsInteger();
   }
   return --institutID;
}
...
static BOOL first_time=TRUE;
   ...
   if (first_time)
   {
	  ...
	  local.BindParam(3,_data.LNAME,50);
...
//
//////////////////////////////////////////////////////////////////////////////
LONG addInstitute(SUP_INSTITUT &data)
{
#define SQL_ADDINSTITUTE "INSERT INTO INSTITUT  (INSTITID, PINSID, INSNAME, INSACR, INSTYPE, STREET, POSTBOX, CITY, STATEID, " \
	                      " CPOSTAL, CNTRYID, APHONE, AFAX, AEMAIL, WEBURL, SINS, EINS, ICHANGE )VALUES " \
		            " (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?) "
	
	PUSH(addInstitute);

static CODBCbindedStmt &local  = _localDMS->BindedStmt(SQL_ADDINSTITUTE);

static SUP_INSTITUT _data;
static BOOL first_time=TRUE;

   data.INSTITID = getNextPersonID();
   _data = data; 
   if (first_time)
   {
      local.BindParam(1,_data.INSTITID);
	  local.BindParam(2, _data.PINSID);
	  local.BindParam(3,_data.INSNAME, 152);
	  local.BindParam(4,_data.INSACR,24);
      local.BindParam(5,_data.INSTYPE);
	  local.BindParam(6,_data.STREET,128);
	  local.BindParam(7,_data.POSTBOX,28);
	  local.BindParam(8,_data.CITY,36);
	  local.BindParam(9,_data.STATEID);
      local.BindParam(10,_data.CPOSTAL,12);
      local.BindParam(11,_data.CNTRYID);
      local.BindParam(12,_data.APHONE,28);
      local.BindParam(13,_data.AFAX,28);
	  local.BindParam(14,_data.AEMAIL,44);
	  local.BindParam(15,_data.WEBURL,64);
	  local.BindParam(16,_data.SINS);
	  local.BindParam(17,_data.EINS);
	  local.BindParam(18,_data.ICHANGE);
   ...
   if (fOpt == FIND_FIRST) {
      ...
      if (first_time)
	  {
        ...
        BIND(1,_data.PERSONID);
  	    BINDS(2,_data.FNAME, 24);
	    BINDS(3,_data.LNAME,52);
        BINDS(4,_data.IONAME,16);
	    BIND(5,_data.INSTITID);
	    BINDS(6,_data.PTITLE,28);
	    BINDS(7,_data.PONAME,52);
	    BIND(8,_data.PLANGU);
        BINDS(9,_data.PPHONE,24);
        BINDS(10,_data.PEXTENT,24);
        BINDS(11,_data.PFAX,24);
	    BINDS(12,_data.PEMAIL,44);
	    BIND(13,_data.PROLE);
	    BIND(14,_data.SPERSON);
	    BIND(15,_data.EPERSON);
	    BIND(16,_data.PSTATUS);
	    BINDS(17,_data.CONTACT,256);
...
{
	PUSH(SUP_addInstitute);
	ret = addInstitute(*data);
	POP();
}
...
LONG DLL_INTERFACE SUP_getPersonInst(long institid, char fname[24], char lname[52], char insacr[24],char title[28], long* personid, long fOpt)
{
   ...
   if (fLocal){
      ...
      if (ret!=DMS_SUCCESS){
         fLocal = FALSE; 
         ret=getPersonInst(CENTRAL,institid,  fname, lname,  insacr,title, personid, FIND_NEXT);
      }
   }
   else
